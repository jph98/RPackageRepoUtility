...
def build_all_packages():
	...
	if os.path.isfile(packages_file):
		print "Performing backup of old PACKAGES file"
	...
	print "Found " + str(len(files)) + " files to add"
	for f in files:		
...
if  __name__ =="__main__":
	...
	parser = OptionParser()	
	parser.add_option("-b", action="store_true", dest="buildall", default=False, help="Rebuild of PACKAGES based on current directory")
	parser.add_option("-a", dest="packagename", help="Append a packagename to existing PACKAGES file (e.g. ggplot2_0.9.3.1.tar.gz)")
	(options, args) = parser.parse_args()
	
	if options.buildall:
		print "Building package index for all"
		build_all_packages()
